...
public class PatientController {
    ...
    @PostMapping
    public ResponseEntity<Patient> createPatient(@RequestBody PatientDTOCreationRequest patientDTOCreationRequest) {
        try {
            log.info("PatientController >> createPatient >> patientDTOCreationRequest:: {}", patientDTOCreationRequest);
            Patient persistedPatient = patientService.registerPatient(patientDTOCreationRequest);
            ...
            log.warn("PatientController >> createPatient >> Failed to create new patient: {}", patientDTOCreationRequest, e);
            return ResponseEntity.badRequest().build();
        } catch (UnsupportedMediaType e) {
            log.warn("PatientController >> createPatient >> Unsupported media type: {}", patientDTOCreationRequest, e);
